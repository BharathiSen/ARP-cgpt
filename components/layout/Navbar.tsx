...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 py-4 px-6 md:px-12 transition-all duration-300" 
         style={{ background: 'rgba(2, 6, 23, 0.85)', backdropFilter: 'blur(24px)', borderBottom: '1px solid rgba(0,200,255,0.08)' }}>
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2 group cursor-pointer" onClick={() => { if(pathname==='/') window.scrollTo({top:0, behavior:'smooth'}); }}>
        ...
        {/* Desktop Menu */}
        <div className="hidden md:flex items-center gap-8 text-sm font-medium" style={{ color: '#9AA6C4' }}>
          {links.map((link) => (
            <button
              key={link.name} 
              onClick={() => handleNavClick(link.href)}
              className="transition-colors cursor-pointer"
              style={{ color: activeSection === link.href ? '#00C8FF' : '#9AA6C4' }}
            >
